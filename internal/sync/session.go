...
func (s *Session) SyncWithPeer(ctx context.Context, pi peer.AddrInfo) error {
	...
	return s.exchange(ctx, stream)
}

func (s *Session) handleStream(stream network.Stream) {
	defer stream.Close()

	s.exchange(context.Background(), stream)
}

func (s *Session) exchange(ctx context.Context, stream network.Stream) error {
	localData, err := collectLocalData(
		database.NewPlaylistRepo(s.db),
		database.NewSubscriptionRepo(s.db),
	)
	if err != nil {
		return fmt.Errorf("collect local data: %w", err)
	}

	if err := encodeJSONMessage(stream, localData); err != nil {
		return fmt.Errorf("send local data: %w", err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("receive remote data: %w", err)
	}

	if err := MergeIncomingData(s.db, remoteMsg); err != nil {
		return fmt.Errorf("merge remote data: %w", err)
	}
